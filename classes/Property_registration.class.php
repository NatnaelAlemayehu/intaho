...
class Property_registration extends Database
{
    ...
    private function getpostdata()
    {
        ...
        if ($propertyId){
            $this->uploadbedroompic($propertyId, $propertyId+1);
            $this->uploadbathroompic($propertyId, $propertyId+2);
    ...
    private function uploadbedroompic($propertyId, $imgId){
            ...
            if (in_array($fileActualExt, $allowed)) {
                if ($fileError === 0) {
                    if ($fileSize < 5250000) {
                        $fileNameNew = "bedroom". $fileExt[0]."$imgId".".".$fileActualExt;
    ...
    private function uploadbathroompic($propertyId, $imgId){
        ...
        if (in_array($fileActualExt, $allowed)) {
            if ($fileError === 0) {
                if ($fileSize < 5250000) {
                    $fileNameNew = "bathroom" . $fileExt[0] . "$imgId" . "." . $fileActualExt;
                    $filedestination = '../uploads/' . $fileNameNew;
                    move_uploaded_file($fileTmpName, $filedestination);
